...
})('Wildpad', function () {var wildpad = wildpad || { };
wildpad.utils = { };
...
wildpad.ACEAdapter = (function() {
  ...
  ACEAdapter.prototype.operationFromACEChange = function(change) {
    ...
    operation = new wildpad.TextOperation().retain(start).insert(text).retain(restLength);
    inverse = new wildpad.TextOperation().retain(start)["delete"](text).retain(restLength);
    if (action === 'remove') {
...
wildpad.EntityManager = (function () {
  ...
  EntityManager.prototype.exportToElement = function(entity) {
    ...
    elt.setAttribute('data-wildpad-entity', entity.type);
    return elt;
  };
  ...
  */
  ...
  EntityManager.prototype.fromElement = function(element) {
    var type = element.getAttribute('data-wildpad-entity');
    ...
    if (type && this.entities_[type]) {
      ...
      return new wildpad.Entity(type, info);
    }
  };
  ...
  EntityManager.prototype.tryRenderToElement_ = function(entity, renderFn, entityHandle) {
    ...
    if (this.entities_[type] && this.entities_[type][renderFn]) {
      var windowDocument = wildpad.document || (window && window.document);
      ...
      if (res) {
        if (typeof res === 'string') {
          ...
          wildpad.utils.assert(typeof res.nodeType !== 'undefined', 'Error rendering ' + type + ' entity.  render() function' +
...
wildpad.Wildpad = (function(global) {
  ...
  Wildpad.prototype.addPoweredByLogo_ = function() {
    ...
    poweredBy.setAttribute('href', 'https://www.wilddog.com');
